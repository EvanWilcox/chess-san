...
function App() {
  const [config, setConfig] = useState({fen: board.fen(), orientation: getColor(board), movable: {free: false, dests: toDests(board)}, draggable: {showGhost: true}, events: {move: playerMove()}, highlight: {lastMove: false}});
  ...
  function playerMove() {
    return (orig: Key, dest: Key, capturedPiece?: Piece) => {
      ...
      // Reset Board
      setConfig({fen: board.fen(),orientation: getColor(board),movable: {free: false,dests: toDests(board)},draggable: {showGhost: true}, events: {move: playerMove()}, highlight: {lastMove: false}
      })
    }
  }
